...
export default function ChatTextBar(){
    ...
    return(
        <div className="w-full py-2 px-2 mt-2 gap-2 flex items-center justify-between sticky bottom-0 border-t bg-transparent border-slate-800 " >
            <div className="flex grow w-1/2 min-h-12 items-center rounded-3xl gap-2 px-2 bg-slate-700" >
                ...
                <textarea ref={textareaRef} className="flex grow h-7 max-h-72 w-1/2 text-white text-xl scrollbar-none resize-none bg-transparent outline-none caret-green-500 placeholder:text-gray-400" placeholder="Message" />
            ...
            <span className="material-symbols-rounded cursor-pointer flex items-center justify-center w-12 h-12 text-2xl text-slate-800 bg-green-500 rounded-full icon-fill" >{hasValue ? "send" : "mic"}</span>
        </div>
    )
}